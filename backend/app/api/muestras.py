...
def _build_estados(settings) -> tuple[pd.DataFrame, list[str]]:
    try:
        ...
        area2, errores_area2 = validate_rows(area2, AnalisisRow)
    ...
    area2["id_muestra"] = correct_ids(
        area2["id_muestra"], master_ids, threshold=settings.fuzzy_correct_threshold
    )

    return build_status(checklist, area2), errores_checklist + errores_area2
